const log = require('loglevel').getLogger('RankCommand'),
  ...
  class PvPRankingData {
	...
	async getUserRequest(message, userCommand){
		...
		if(typeof(inputFilter[0]) == 'string'){ inputFilter[0] = inputFilter[0].toLowerCase();}
		...
		this.inputAttackIV = await this.atkCollector.obtain(message, inputAttackIV); //Sees if Attack IV argument was included. If not, it prompts user for one.
		if (!this.inputAttackIV.cancelled) {
			this.inputAttackIV = this.inputAttackIV.values.ATK_IV;
		} else {
			this.flag = true; 
			return; 
		}
		this.inputDefenseIV = await this.defCollector.obtain(message, inputDefenseIV); //Sees if Defense IV argument was included. If not, it prompts user for one.
		if (!this.inputDefenseIV.cancelled) {
			this.inputDefenseIV = this.inputDefenseIV.values.DEF_IV;
		} else {
			this.flag = true; 
			return; 
		}
		this.inputStaminaIV = await this.staCollector.obtain(message, inputStaminaIV); //Sees if Stamina IV argument was included. If not, it prompts user for one.
		if (!this.inputStaminaIV.cancelled) {
			this.inputStaminaIV = this.inputStaminaIV.values.STA_IV;
		} else {
			this.flag = true; 
			return; 
		}
		...
		if(this.command == 'great-evo' || this.command == 'ultra-evo'){
	...
	generateRanks(CpmData) {
		...
		for(let i=0; i<this.familyList.length; i++){
			...
			let baseAtk = this.familyList[i].atk,
				baseDef = this.familyList[i].def,
				baseSta = this.familyList[i].sta;
	...
	async displayInfo(message, command) {
		if (this.flag == true) {//If somebody cancels the command in scrape(), we don't want this function running.
			return; 
		};
		...
		let league = '';
		if (command == 'rank' || command == 'great-evo') {
			league = 'GREAT';
		} else if(command == 'ultra-evo'){
			league = 'ULTRA';
		}
		else{
		...
		if (!this.embedErrorMessage) { //If no error message was found.
			...
			console.log("Test2");
			console.log(this.embedName,"is a ",typeof(this.embedName));
			let nameField = `**${this.embedName.replace(/_/g,' ').titleCase()}**  ${this.inputAttackIV}/${this.inputDefenseIV}/${this.inputStaminaIV}\n`; //nameField is pokemon name & IVs.
		...
		let userCommand = `${message.client.commandPrefix}${command}`, //Grabs the !great, !ultra or w/e from user inputs.
			...
			if (this.ivFilterCommand && !(this.ivFilterCommand == "stats" && this.familyList.length > 1)) { userIVString += ` ${this.ivFilterCommand}`}
...
class RankCommand extends Commando.Command {
	...
	async run(message, args) {
		...
		if (userCommand == 'great' || userCommand == 'rank' || userCommand == 'great-evo'){
		...
		else if (userCommand == 'ultra' || userCommand == 'ultra-evo' || userCommand == 'master'){
