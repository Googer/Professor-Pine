const log = require('loglevel').getLogger('Counters'),
    ...
    PokemonData = require('../../data/pokemon')
...
class CountersCommand extends Commando.Command {
    ...
    parseSetBoss(raid) {
        ...
        boss = boss.replace(/[^\w\s]/gi, ' ').replace(/\s{2,}/gi, ' ').toUpperCase();
    ...
    async run(message, args) { 
        ...
        pokebattlerId = await db.DB('User')
            ...
            .pluck('pokebattler_id')
            .first()
            .then(res => {
                if (!!res) {
                    return res.pokebattler_id
                } else {
                    return false
                }
            });
        ...
        for (pokeIdx = 0; pokeIdx < moveset.length; pokeIdx++) {
            ...
            for (moveIdx = 0; moveIdx < pokemon.byMove.length; moveIdx++) {
                ...
                counters.push({pokemon: pokemonName,
        ...
        if (newId) {
            ...
            if (!!user) {
                ...
                await db.DB('User')
                    .where('userSnowflake', message.author.id)
                    .update('pokebattler_id', level.pbName)
            } else {
                // insert
                await db.DB('User')
                    .insert({userSnowflake: message.author.id,
                             pokebattler_id: level.pbName})
            }
