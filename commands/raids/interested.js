...
class InterestedCommand extends Commando.Command {
  ...
  async run(message, args) {
    ...
    if (currentStatus === PartyStatus.NOT_INTERESTED && groupCount > 1 && groupId === false) {
      ...
      statusPromise = Promise.all([
        await raid.setMemberGroup(message.member.id, groupId),
        await raid.setMemberStatus(message.member.id, PartyStatus.INTERESTED, additional)]);
    } else {
      statusPromise = Promise.resolve(
        await raid.setMemberStatus(message.member.id, PartyStatus.INTERESTED, groupId ? 0 : additionalAttendees));
    }
