...
class RegisterFriendCodeCommand extends Commando.Command {
  constructor(client) {
    ...
    client.dispatcher.addInhibitor(message => {
      if (!!message.command && message.command.name === 'register-friend-code' && !Helper.isBotChannel(message)) {
        return {
          reason: 'invalid-channel',
          response: message.reply(Helper.getText('register-friend-code.warning', message))
        };
      }
      return false;
    });
