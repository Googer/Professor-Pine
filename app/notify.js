...
class Notify {
  ...
  async shout(message, members, text, type, fromMember = null) {
    ...
    message.channel.send(membersString, embed)
      .then(message => message.channel.send(`To enable or disable these notifications, use the \`${message.client.commandPrefix}mentions\`, \`${message.client.commandPrefix}mentions-groups\`, and \`${message.client.commandPrefix}mentions\` commands in ${botLabChannel.toString()}.`))
      .catch(err => log.error(err));
  }
}
