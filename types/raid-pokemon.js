    ...
    const validPokemon = pokemon
      .find(pokemon => {
        if (!message.command || message.command.name === 'raid' || message.command.name === 'boss') {
          return pokemon.exclusive || pokemon.elite || pokemon.mega || (pokemon.tier && pokemon.tier < 7);
        }
        return pokemon.exclusive || pokemon.elite || pokemon.mega || pokemon.tier || (pokemon.name.toLocaleLowerCase().startsWith('unown') && settings.roles.unown && settings.channels.unown);
      });
